...
function extractQuotedConst(src, name) {
  const marker = 'const ' + name + ' = "';
  const start = src.indexOf(marker);
  if (start < 0) throw new Error('未找到 `' + marker + '` 标记');
...
function extractData(src) {
  try {
    return extractConst(src, 'DATA');
  } catch (error) {
    const encoded = extractQuotedConst(src, '__DATAZ');
    return JSON.parse(
...
function transformStage(st, boardProblems, ctx, warns) {
  for (const k of CELL_KEYS) {
    if (!(k in st.cells))
      warns.push(`${ctx}: seg「${st.seg}」missing cell ${k}`);
  }
  ...
  for (const day of repo.days) {
    ...
    for (const p of b.problems || []) {
      if (!segs.includes(p.seg))
        warns.push(
          `${ctx}: problem.seg「${p.seg}」has no matching journey seg`
        );
    }
    boards[day] = {
      day,
...
function main() {
  ...
  console.log(
    `[appendix] dict ${DICT.version} (${DICT.updated}) groups=${DICT.groups.length} rows=${dictRows}; ` +
      `taxonomy cls=${TAXONOMY.rows.length} window=${
        TAXONOMY.window
      } -> ${path.relative(process.cwd(), OUT_APPENDIX)} (${
        fs.statSync(OUT_APPENDIX).size
      } bytes)`
  );
