    ...
    ? data
        .filter((item) => {
          if (Array.isArray(item)) {
            return (
              !isHiddenCountryName(item[0]) && !isHiddenCountryName(item[1])
            );
          }
          return (
            !isHiddenCountryName(item?.from) && !isHiddenCountryName(item?.to)
          );
        })
        .map((item) => {
...
const Dependencywheel = () => {
  const { t } = useTranslation('os-situation');
  const yearList = [2025, 2024, 2023, 2022].flatMap((year) => [
    {
      label: `${year}`,
      value: `/test/collaborator_model/dependency_wheel_${year}_AS_country.json`,
    },
    ...[4, 3, 2, 1].map((quarter) => ({
      label: `${year}-Q${quarter}`,
      value: `/test/collaborator_model/dependency_wheel_${year} Q${quarter}_QS-JAN_country.json`,
    })),
  ]);
  const [label, setLabel] = useState(yearList[0].label);
  const [selectedYear, setYear] = useState(yearList[0].value);
