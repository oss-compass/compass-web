...
const CommunityTab: React.FC = () => {
  ...
  const handleBatchQueue = () => {
    ...
    Modal.confirm({
      title: '确认批量操作',
      content: `确定要将 ${targetRepositories.length} 个${batchTimeCategory}的社区加入${queueTypeText}吗？`,
      onOk: () => {
        // 这里可以调用实际的API
        message.success(
          `成功将 ${targetRepositories.length} 个社区加入${queueTypeText}`
        );
        ...
        title={
          <div>
            ...
            <span className="ml-2 text-sm font-normal text-gray-500">
              总计: {totalRepositories.toLocaleString()} 社区
            </span>
          </div>
        }
        className="mt-4"
