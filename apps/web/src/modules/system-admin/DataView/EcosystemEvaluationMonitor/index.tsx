...
const EcosystemEvaluationMonitor: React.FC = () => {
  ...
  const generateQueueData = () => {
    const queues = [];
    for (let i = 1; i <= 48; i++) {
      const projectCount = Math.floor(Math.random() * 200); // 0-19个项目
      const status =
        projectCount === 0
          ? 'idle'
          : projectCount < 5
      ...
      <Card title="队列监控" className="mb-6">
        ...
        <div className="grid grid-cols-8 gap-2">
          {queueData.map((queue) => (
            ...
            >
              <div
                className="relative cursor-pointer rounded border p-3 transition-shadow hover:shadow-md"
                style={{
