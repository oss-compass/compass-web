...
const GraduationManagement: React.FC = () => {
  ...
  const apiPlatform = useMemo(() => {
    ...
    const mapped = platformMapping[platformFilter] ?? 'all';
    return mapped !== 'all' ? mapped : undefined;
  }, [platformFilter]);

  const resetToFirstPage = () => setCurrentPage(1);

  // 包装过滤器设置函数，自动重置到第一页
  const handleUpdateTimeFilterChange = (v: string) => {
    setUpdateTimeFilter(v);
    resetToFirstPage();
  };

  const handlePlatformFilterChange = (v: string) => {
    setPlatformFilter(v);
    resetToFirstPage();
  };
              ...
              <div className="space-y-2">
                <Radio value="超过1个月">
                  ...
                  <span className="ml-2 text-gray-500">
                    (
                    {incubationUpdateOverview?.updated_within_three_months || 0}{' '}
                    个项目)
                  </span>
                </Radio>
                <Radio value="超过3个月">
                  超过 3 个月未更新
                  <span className="ml-2 text-gray-500">
                    ({incubationUpdateOverview?.updated_within_six_months || 0}{' '}
                    个项目)
                  </span>
                </Radio>
                <Radio value="超过半年">
                  ...
                  <span className="ml-2 text-gray-500">
                    (
                    {incubationUpdateOverview?.updated_within_twelve_months ||
                      0}{' '}
                    个项目)
                  </span>
                </Radio>
                <Radio value="超过1年">
                  超过 1 年未更新
                  <span className="ml-2 text-gray-500">
                    ({incubationUpdateOverview?.updated_over_twelve_months || 0}{' '}
                    个项目)
                  </span>
                </Radio>
              </div>
