...
interface PrRecord {
  ...
  numReviewComments: number;
  labels: string[];
  userLogin: string;
  reviewersLogin: string[];
...
const STATE_OPTIONS = [
  ...
  { value: 'merged', label: 'analyze:metric_detail:merged' },
  { value: 'rejected', label: 'analyze:metric_detail:rejected' },
];

const LABELS = ['feature', 'bugfix', 'refactor', 'docs', 'test', 'ci'];
...
// 生成 mock PR 数据
const generateMockPrs = (dashboardId: string, project: string): PrRecord[] => {
  const seed = hashSeed(`${dashboardId}-${project}-prs`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    const reviewerSeed1 = (seed + i * 17) % USERS.length;
    const reviewerSeed2 = (seed + i * 19) % USERS.length;
    const mergeSeed = (seed + i * 23) % USERS.length;
    ...
    const createdDaysAgo = 30 + ((seed + i * 29) % 60);
    const createdAt = new Date(baseDate);
    createdAt.setDate(createdAt.getDate() - createdDaysAgo);

    const closedAt = state !== 'open' ? new Date(createdAt) : null;
    if (closedAt) {
      closedAt.setDate(closedAt.getDate() + 2 + ((seed + i * 31) % 8));
    }
    ...
    const reviewers = [USERS[reviewerSeed1]];
    if ((seed + i) % 2 === 0) {
      reviewers.push(USERS[reviewerSeed2]);
    }

    prs.push({
      id: `${project}-pr-${i}`,
      title: `PR #${2000 + i}: ${
        ['Implement', 'Fix', 'Refactor', 'Update'][i % 4]
      } ${['feature', 'bug', 'module', 'docs'][i % 4]} in ${project
        .split('/')
        .pop()}`,
      url: `https://${
        project.includes('gitee')
          ...
          ? 'gitcode.com'
          : 'github.com'
      }/${project.replace(/^(github:|https?:\/\/[^/]+\/)/, '')}/pull/${
        2000 + i
      }`,
      state,
      ...
      timeToFirstAttentionWithoutBot:
        state !== 'open' ? 0.3 + ((seed + i * 37) % 20) / 10 : null,
      numReviewComments: (seed + i * 41) % 20,
      labels,
      userLogin: USERS[userSeed],
      reviewersLogin: reviewers,
      mergeAuthorLogin: state === 'merged' ? USERS[mergeSeed] : null,
    });
  }
...
const generateMockStats = (dashboardId: string, project: string) => {
  const seed = hashSeed(`${dashboardId}-${project}-pr-stats`);
  const pullCount = 15 + (seed % 40);
  const completedCount = Math.floor(pullCount * (0.6 + (seed % 30) / 100));
  const unresponsiveCount = seed % 5;
  const commitCount = 50 + (seed % 150);

  return {
    pullCount,
    pullCompletionRatio: completedCount / pullCount,
    pullCompletionCount: completedCount,
    pullUnresponsiveCount: unresponsiveCount,
    commitCount,
  };
};
...
const PrTable: React.FC<PrTableProps> = ({
  ...
  const tableData = useMemo(() => {
    ...
    return generateMockPrs(dashboardId, selectedProject);
  ...
  const columns: ColumnsType<PrRecord> = [
    ...
    },
    {
      ...
      align: 'left',
      width: 100,
      sorter: (a, b) => a.numReviewComments - b.numReviewComments,
    },
    {
  ...
  return (
    <BaseCard
      id="pr_table"
      title={String(t('os_board:detail.pr_table'))}
      bodyClass=""
    >
      {/* 项目切换 Tabs */}
      <Tabs
