...
import useI18RangeTag from '@modules/analyze/components/NavBar/useI18RangeTag';
...
import useOsBoardSwitchRange from '../../hooks/useOsBoardSwitchRange';
...
const DateTagPanel = ({
  ...
  const { range } = useOsBoardDateRange();
  const { switchRange } = useOsBoardSwitchRange();
...
const DashboardDatePicker = ({ disable }: DashboardDatePickerProps) => {
  const i18RangeTag = useI18RangeTag();
  const { range } = useOsBoardDateRange();
