...
import { useTpcSoftwareSandboxReportRiskQuery } from '@oss-compass/graphql';
import {
  setRiskLoading,
  setRiskData,
  RiskStore,
...
const RiskFetcher = ({ shortCode }) => {
  const { data, isLoading, refetch } = useTpcSoftwareSandboxReportRiskQuery(
    client,
    {
      shortCode,
    }
  ...
  useEffect(() => {
    setRiskData(shortCode, data?.tpcSoftwareSandboxReport);
  }, [data]);
  useEffect(() => {
