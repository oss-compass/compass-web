...
import { useCreateTpcSoftwareSandboxReportMutation } from '@oss-compass/graphql';
import getErrorMessage from '@common/utils/getErrorMessage';
import HasOhRole from '@modules/oh/components/HasOhRole';
import { toast } from 'react-hot-toast';

const SandboxReportApplication = () => {
  const [form] = Form.useForm();
  const mutation = useCreateTpcSoftwareSandboxReportMutation(client, {
    onSuccess(data) {
      if (data.createTpcSoftwareSandboxReport.status == 'true') {
        toast.success(`提交成功，可在沙箱项目申请列表中查看报告状态！`);
        setTimeout(() => {
          window.location.hash = 'sandboxTable?tab=1';
        }, 2000);
      } else {
        toast.error(data.createTpcSoftwareSandboxReport.message);
      }
    },
    onError(res) {
...
export default SandboxReportApplication;
