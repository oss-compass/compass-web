...
import {
  allMetricData,
  getRishContent,
} from '@modules/oh/DataView/HatchingTreatment/Sandbox/EvaluationInfo/AllSandboxMetricData';
...
export const getEvaluationDetail = (row) => {
  const evaluationDetail = metricList.map((item) => {
    const d = allMetricData.filter((i) => {
      let hasScore =
        row?.tpcSoftwareSandboxReportMetric?.[i.key] !== null &&
        row?.tpcSoftwareSandboxReportMetric?.[i.key] >= 0;
      return i.维度 === item && hasScore;
    });
    let scoreTotal = 0;
    d.forEach((i) => {
      scoreTotal += row?.tpcSoftwareSandboxReportMetric?.[i.key] || 0;
    });
    const score: number =
...
export const downloadReport = (item) => {
  const { tpcSoftwareSandboxReportMetric, name, tpcSoftwareReportMetricRaw } =
    item;
  const metricScore = getMetricItemScore(tpcSoftwareSandboxReportMetric);
  let title = name + '评估报告';
  let head = [
    '指标名称',
    '维度',
