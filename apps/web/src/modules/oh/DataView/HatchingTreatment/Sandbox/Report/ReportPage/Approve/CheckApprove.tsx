...
import { useAcceptTpcSoftwareSandboxMutation } from '@oss-compass/graphql';
...
import {
  ...
  ReportStore,
  ReportEvent,
} from '@modules/oh/DataView/HatchingTreatment/Sandbox/Report/ReportPage/store/useReportStore';
...
const CheckApprove = ({ selectionId }) => {
  ...
  const { commentSigLeadPermission, commentLegalPermission, commentState } =
    useGetReportData();
  ...
  const mutation = useAcceptTpcSoftwareSandboxMutation(gqlClient);
  const handleApprove = (memberType, state) => {
    mutation.mutate(
      {
        sandboxId: selectionId,
        memberType,
        state,
      },
      {
        onSuccess: (res) => {
          if (res.acceptTpcSoftwareSandbox.status === 'true') {
            ReportStore.event$?.emit(ReportEvent.REFRESH);
            toast.success(`${state === 0 ? '取消' : '操作'}成功`);
          } else {
            toast.error(res.acceptTpcSoftwareSandbox.message);
          }
        },
      }
    );
  ...
  const hasCommentPermissions = () => {
    return commentSigLeadPermission || commentLegalPermission;
  };
