...
import { useUpdateTpcSoftwareSandboxReportMutation } from '@oss-compass/graphql';
...
const EditReportForm = ({ report, refetch }) => {
  ...
  const mutation = useUpdateTpcSoftwareSandboxReportMutation(client, {
    onSuccess(data) {
      if (data.updateTpcSoftwareSandboxReport.status == 'true') {
        toast.success('修改成功');
        refetch && refetch();
      } else {
        toast.error(data.updateTpcSoftwareSandboxReport.message);
      }
    },
    onError(res) {},
  });
