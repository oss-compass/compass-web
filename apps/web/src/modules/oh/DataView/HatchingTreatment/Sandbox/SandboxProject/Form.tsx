                      ...
                      onChange={(value) => {
                        form.setFieldsValue({
                          committers: report
                            ...
                            .join(', '),
                          // adaptationCommitters: undefined,
                        });
                      }}
                      disabled={false}
                    >
                    ...
                    <>
                      ...
                      {/* <Form.Item
                        ...
                        shouldUpdate={(prevValues, currentValues) =>
                          prevValues.targetSoftware !== currentValues.targetSoftware
                        }
                      >
                        {({ getFieldValue }) => {
                          const targetSoftware = getFieldValue('targetSoftware');
                          const selectedReport = report.find((item) =>
                            item.codeUrl.includes(targetSoftware)
                          );
                          const adaptationCommitters =
                            selectedReport?.tpcSoftwareSig?.adaptationCommitter ||
                            [];
                          const hasAdaptationCommitters =
                            adaptationCommitters.length > 0;

                          return hasAdaptationCommitters ? (
                            <Col span={24}>
                              <Form.Item
                                labelCol={{
                                  span: 3,
                                ...
                                label="仓库 Committers"
                                name="adaptationCommitters"
                                rules={[
                                  {
                                    required: true,
                                    message: '请至少选择一个仓库 Committer!',
                                  },
                                ]}
                              >
                                <Select
                                  mode="multiple"
                                  placeholder="请选择仓库 Committers"
                                >
                                  {adaptationCommitters.map((committer) => (
                                    <Select.Option
                                      key={committer.id}
                                      value={committer.gitcodeAccount}
                                    >
                                      {committer.name
                                        ? `${committer.name}(${committer.gitcodeAccount})`
                                        : committer.gitcodeAccount}
                                    </Select.Option>
                                  ))}
                                </Select>
                              </Form.Item>
