...
import { useTpcSoftwareSandboxQuery } from '@oss-compass/graphql';
...
const ReportFetcher = ({ selectionId }) => {
  const { data, isLoading, refetch } = useTpcSoftwareSandboxQuery(client, {
  ...
  useEffect(() => {
    setReportData(data?.tpcSoftwareSandbox);
  }, [data]);
