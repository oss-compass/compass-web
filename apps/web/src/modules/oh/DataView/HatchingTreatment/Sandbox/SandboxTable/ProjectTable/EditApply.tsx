...
import { useUpdateTpcSoftwareSandboxMutation } from '@oss-compass/graphql';
...
const EditReportForm = ({ report, refetch }) => {
  ...
  const name = report?.tpcSoftwareSandboxReports[0].name;
  ...
  const mutation = useUpdateTpcSoftwareSandboxMutation(client, {
    onSuccess(data) {
      if (data.updateTpcSoftwareSandbox.status == 'true') {
        toast.success('修改成功');
        refetch && refetch();
      } else {
        toast.error(data.updateTpcSoftwareSandbox.message);
      }
    },
    onError(res) {},
  });
    ...
    <div className="oh-tabs px-6">
      ...
      >
        <Row gutter={24}>
          ...
          <Col span={12} className="relative">
            {sameCheck ? (
              <Form.Item label="同类型三方库链接" name="sameTypeSoftwareName">
