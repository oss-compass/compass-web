...
const IssueExperiencePath: React.FC<IssueExperiencePathProps> = ({
  ...
  useEffect(() => {
    ...
    document
      .getElementById(`issue-stage-card-${activeStageId}`)
      ?.scrollIntoView({
        block: 'nearest',
        inline: 'center',
      });
    ...
    >
      <div className="mb-3">
        ...
        <p className="mt-1.5 text-xs leading-5 text-slate-500">
          点击阶段卡片展开该阶段的诊断详情（关键指标、痛点与代表 Issue）；G 为 Bot / Agent
          治理参考镜头，不计入总分。
        </p>
      </div>
      ...
      <div className=">md:px-6 rounded-[28px] border border-slate-200 bg-white px-4 py-5 shadow-[0_18px_42px_rgba(15,23,42,0.06)]">
        ...
        <div className="mt-6 overflow-x-auto pb-2 pt-1 [scrollbar-width:thin]">
          <div
            role="tablist"
          ...
          <div className=">md:px-5 border-b border-slate-200 bg-white px-4 py-4">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="shrink-0 text-right">
                ...
                <div className={`mt-1 text-3xl font-bold ${scoreTone.text}`}>
                  {activeStage.mixed}
                </div>
              </div>
            </div>
            ...
            <div className=">md:p-5 border-t border-slate-200 bg-white/70 p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  ...
                  {activeStage.low_issues.length > 3 ? '（展示前 3 条）' : ''}
                </span>
              </div>
