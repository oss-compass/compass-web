...
import type { CiTrendItem } from './communityMetrics';

const { Title } = Typography;

type CiTrendModalProps = {
  open: boolean;
  trend: CiTrendItem | null;
  days: string[];
  onClose: () => void;
};
...
/**
 * 关键指标趋势 · 弹窗大图（纯 SVG 折线图）。
 * 复用总览缩略图的折线/渐变观感，补充坐标轴、网格与数值点，供点击缩略图后查看。
 */
const CiTrendModal: React.FC<CiTrendModalProps> = ({
  open,
  trend,
  days,
  onClose,
        ...
        <>
          <div className="mb-1 flex items-baseline gap-3">
            ...
            >
              最新{' '}
              {trend.last != null
                ? Number.isInteger(trend.last)
                  ? trend.last
                  : trend.last.toFixed(1)
                : '—'}
              {trend.unit}
            </span>
            {trend.delta.cls !== 'flat' && trend.delta.txt ? (
              <span
                ...
                style={{
                  color:
                    trend.delta.cls === 'bad'
                      ? '#d03b3b'
                      : trend.delta.cls === 'good'
                      ? '#16835e'
                      : '#64748b',
                }}
              >
                {trend.delta.arrow} {trend.delta.txt} · {trend.delta.word}
              </span>
            ) : null}
          </div>
