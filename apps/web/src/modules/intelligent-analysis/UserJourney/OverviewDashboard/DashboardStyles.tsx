...
const dismissDashboardOverlays = () => {
  if (typeof document === 'undefined') return;
  // 表头筛选既有 Ant Design 浮层，也有监听 document.mousedown 的自定义浮层。
  // 浏览器 chrome 不属于页面 DOM，窗口失焦时补发一次真正的“外部点击”语义。
...
const DashboardStyles: React.FC<DashboardStylesProps> = ({
  ...
  useEffect(() => {
    const handleWindowBlur = () => dismissDashboardOverlays();
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden') dismissDashboardOverlays();
    };
